...
3. **Run the scraper**. The example below writes to a sheet named `Shopee Rojak Dataset` and stops after processing 30 popular products. The command adds the `src/` directory to `PYTHONPATH` so Python can find the package without installing it:
